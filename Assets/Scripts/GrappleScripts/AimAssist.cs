...
public class AimAssist : MonoBehaviour
{
    ...
    public Collider HookDetection(Vector3 center, float radius)
    {
        ...
        {
            if (((LayerMask.GetMask("Grapple") & 1 << hitCollider.gameObject.layer) > 0) || 
                ((LayerMask.GetMask("GrappleYank") & 1 << hitCollider.gameObject.layer) > 0))
            {
                ...
                if (hitCollider.bounds.center.y > playerRB.worldCenterOfMass.y)
                {
                    ...
                    // Sets a new closestHook if a 'Grapple' or 'GrappleYank' point is closer than 
                    // the current closestHook
                    if (Vector3.Distance(center, hitCollider.transform.position) <= 
                        Vector3.Distance(center, closestHook.transform.position))
                    {
                        closestHook = hitCollider;
                    }
