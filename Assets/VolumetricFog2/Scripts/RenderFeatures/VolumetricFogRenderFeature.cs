        ...
        class BlurRenderPass : ScriptableRenderPass {
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData) {
                ...
                if (manager.scattering > 0) {
                    ...
                    ComputeScattering(cmd, source, mat, manager.downscaling);
                }
            ...
            const int PYRAMID_COUNT = 4;


#if UNITY_2022_1_OR_NEWER
            void ComputeScattering(CommandBuffer cmd, RTHandle source, Material mat, float downscaling) {
#else
            void ComputeScattering(CommandBuffer cmd, RenderTargetIdentifier source, Material mat, float downscaling) {
#endif
                // Initialize buffers descriptors
                if (rt == null || rt.Length != PYRAMID_COUNT + 1) {
                    rt = new ScatteringMipData[PYRAMID_COUNT + 1];
